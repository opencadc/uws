    ...
    public void testResults() {
        try {
            if (testPropertiesList.propertiesList.size() == 0) {
                Assert.fail("missing properties file for " + CLASS_NAME);
            }
            ...
            for (TestProperties properties : testPropertiesList.propertiesList) {
                ...
                Assert.assertNotNull(properties.filename + " POST response to " + resourceUrl 
                ...
                while (!done) {
                    ...
                    Assert.assertNotNull(properties.filename + " no XML returned from GET of " + resourceUrl, root);

                    // Get the phase element.
                    list = root.getChildren("phase", namespace);
                    Assert.assertEquals(properties.filename 
                    ...
                    if ((System.currentTimeMillis() - start) > (REQUEST_TIMEOUT * 1000)) {
                        Assert.fail(properties.filename + " request timeout exceeded in GET of " + resourceUrl);
                    }

                    // COMPLETED phase, continue with test.
                    if (phaseText.equals("COMPLETED")) {
                        break;
                    } else if (phaseText.equals("ERROR") 
                            || phaseText.equals("ABORTED")) {
                        Assert.fail(properties.filename + " phase should not be " + phaseText 
                                + ", in XML returned from GET of " + resourceUrl);
